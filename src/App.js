...
const MAP_IMAGES = {
  0: '/map.jpg',        // Ground Floor
  1: '/floor1.jpg',     // First Floor
  2: '/floor2.jpg'      // Second Floor
};
...
function App() {
  ...
  useEffect(() => {
    const localData = localStorage.getItem('mapGraph');
    if (localData) {
      try {
        ...
        fetch('/groundfloor.json')
          .then(res => res.json())
          .then(data => {
            setNodes(data.nodes || []);
            setEdges(data.edges || []);
          })
          .catch(() => console.log("No initial map file found."));
    }
  }, []);

  useEffect(() => {
    if (nodes.length > 0) {
      localStorage.setItem('mapGraph', JSON.stringify({ nodes, edges }));
    }
  }, [nodes, edges]);
  ...
  // --- 4. Canvas Rendering (With Arrows) ---
  ...
  return (
    <div className="app-container flex-col">
          ...
          <div className="admin-toolbar">
            ...
            {mode === 'admin' && (
              <div className="admin-edit-row">
                <input type="text" placeholder="Room/Location Name..." value={editForm.name} 
                       onChange={e => setEditForm({...editForm, name: e.target.value})} disabled={!selectedNodeId} />
                <input type="text" placeholder="Details/Professor..." value={editForm.subtitle} 
      ...
      {/* Datalist for native search dropdowns */}
      ...
      {/* Info Card (User Mode Pop-up when a room is clicked) */}
      {mode === 'user' && infoNode && (
        <div className="room-info-card">
          ...
          {infoNode.description && <p>{infoNode.description}</p>}
